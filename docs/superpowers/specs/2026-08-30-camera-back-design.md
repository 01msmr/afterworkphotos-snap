...
| wheel | Ø 72, right 4 %, bottom edge 12 above the LCD | always present: ridged rim, domed centre button with a small rotate-arrows glyph in the site's blue (`#7a7a7a` when inactive). Before a shot: greyed — desaturated and dimmed; the whole panel at 70 % opacity while the print is empty |
